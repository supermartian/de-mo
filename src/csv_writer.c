...
static int write_csv_header(FILE *file) {
    return fprintf(file,
        ...
        "mv_ref_to_cur_x,mv_ref_to_cur_y,flags\n") < 0 ? -1 : 0;
}
...
static int write_csv_vector(
    ...
    int written = fprintf(file,
        "%" PRId64 ",%.9f,%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%u,%.9f,%.9f,0x%" PRIx64 "\n",
        ...
        vector->dx, vector->dy, vector->codec_flags);
    return written < 0 ? -1 : 0;
}

static int write_stats_row(FILE *file, const MvstabTimelineFrame *frame) {
    const FrameMotion *motion = &frame->output;
    int written = fprintf(file, "%" PRId64 ",%.9f,%s,%.9f,%.9f,%.9f,%.6f,%.6f,"
            "%.9f,%.9f,%.6f,%d,%d,%d,%d,%d,%d,%.9f,%.9f,%d,%.6f\n",
...
int mvstab_write_stats_csv_file(
    ...
    if (fprintf(file, "frame_index,pts_seconds,pict_type,dx,dy,theta,confidence,"
